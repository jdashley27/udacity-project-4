...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    // save length in a variable, to save on resources (Source: JavaScript Patterns, by Stoyan Stefanov pages 15-16) Shaved 4 ms
    // Removed common selectors from for loop, as only need to use the loop to paint the DOM
...
function updatePositions() {
  ...
  // Fixed performanc issues by caching the document.body.scrollTop (found by walking through https://www.igvita.com/slides/2012/devtools-tips-and-tricks/jank-demo.html source code)
  var cachedScrollTop = document.body.scrollTop; // using cached script
  var items = document.querySelectorAll('.mover');
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((cachedScrollTop / 1250) + (i % 5)); // using cached event
    //var phase = Math.sin((document.body.scrollTop / 1250) + (i % 5)); // old script
